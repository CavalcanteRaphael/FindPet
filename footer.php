<footer class="page-footer">
    <div class="container">
        <div class="blocofooter row">
                <div class="descrifooter col s12 m6 l3">
                    <h5 class="white-text">FindPet</h5>
                    <p class="grey-text text-lighten-4">Findpet um site voltado <br/> para ajudar seu Pet</p>
                </div>
                <div class="col s12 m6 l3">
                    <h5>Serviços</h5>
                    <ul>
                        <li><a class="grey-text text-lighten-3"  href="adotar.php">Adotar</a></li>
                        <li><a class="grey-text text-lighten-3"  href="doacao.php">Doar</a></li>
                        <li><a class="grey-text text-lighten-3"  href="encontrado.php">Encontrados</a></li>  
                        <li><a class="grey-text text-lighten-3"  href="mapa.php">Mapa dos Pets</a></li>
                    </ul>
                </div>
                <div class="col s12 m6 l3">
                    <h5 class="white-text">Redes Sociais</h5>
                    <ul>
                        <li><a class="grey-text text-lighten-3" href="https://www.facebook.com/FindPetOrg/" target="_blank">Facebook</a></li>
                        <li><a class="grey-text text-lighten-3" href="https://www.instagram.com/findpet_Org/?hl=pt-br" target="_blank">Instagram</a></li>
                        <li><a class="grey-text text-lighten-3" href="https://twitter.com/findpet_org" target="_blank">Twitter</a></li>
                    </ul>
                </div>
                <div class="col s12 m6 l3">
                    <h5>Informações</h5>
                    <ul>
                        <li><a class="grey-text text-lighten-3" href="sobre.php">Sobre</a></li>
                        <li><a class="grey-text text-lighten-3" href="depoimentos.php">Depoimentos</a></li>
                        <li><a class="grey-text text-lighten-3" href="ajude-nos.php">Ajude-nos</a></li>
                    </ul>
                </div>
        </div>
    </div>
    <div class="blue-grey darken-4 footer-copyright">
        <div class="container">
            © 2018 - Findpet
        </div>
    </div>
</footer>